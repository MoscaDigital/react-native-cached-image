...
module.exports = (defaultOptions = {}, urlCache = MemoryCache, fs = fsUtils, path = pathUtils) => {
  ...
  function isCacheable(url) {
    return (
      _.isString(url) &&
      (_.startsWith(url.toLowerCase(), 'http://') || _.startsWith(url.toLowerCase(), 'https://'))
    );
  }
  ...
  return {
    ...
    downloadAndCacheUrl(url, options = {}) {
      return cacheUrl(url, options, (filePath) => fs.downloadFile(url, filePath, options.headers));
    },
    ...
    seedAndCacheUrl(url, seedPath, options = {}) {
      return cacheUrl(url, options, (filePath) => fs.copyFile(seedPath, filePath));
    },
